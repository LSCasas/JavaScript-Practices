...
*/    const obtenerGenerosUnicos = (miArray) => {
  const todosLosGeneros = miArray.map((pelicula) => pelicula.genre);
  const generosUnicos = todosLosGeneros.filter(
    (genero, index, array) => array.indexOf(genero) === index
  );
...
const obtenerDirectoresUnicos = (miArray) => miArray.map((directores) => directores.director);

const directoresUnicos = obtenerDirectoresUnicos(movies);
...
/*
     ...
     const obtenerProtagonistasUnicosOrdenados = (miArray) => {
        const todosLosProtagonistas = movies.flatMap((pelicula) => pelicula.protagonists.map((protagonista) => protagonista.name));
        const protagonistasUnicos = todosLosProtagonistas.filter(
          (protagonista, index, array) => array.indexOf(protagonista) === index
        );
        const protagonistasOrdenados = protagonistasUnicos.sort();
        return protagonistasOrdenados;
      };
      

      const protagonistasUnicosOrdenados = obtenerProtagonistasUnicosOrdenados(movies);
      console.log(protagonistasUnicosOrdenados);
